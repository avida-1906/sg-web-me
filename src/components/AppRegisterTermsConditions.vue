    ...
    <div ref="scrollRef" class="scroll-y terms-conditions">
      <div>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>b) Residing in jurisdictions from where it is illegal or gambling is not permitted. {{ companyData?.name.toUpperCase() }} is not able to verify the legality of the Service in each jurisdiction and it is the User's responsibility to ensure that their use of the Service is lawful;</p>
          <p>c) Provide misleading information or try to pass by third parties.</p>
          <p>4.6 {{ companyData?.name.toUpperCase() }} reserves the right to refuse any application for a {{ companyData?.name.toUpperCase() }} Account, at its sole discretion.</p>
          ...
          <p>4.10 Should {{ companyData?.name.toUpperCase() }} have reason to believe that You have registered and/or used more than one {{ companyData?.name.toUpperCase() }} Account, or colluded with one or more other individuals using a number of different {{ companyData?.name.toUpperCase() }} Accounts, {{ companyData?.name.toUpperCase() }} shall be entitled to deem such accounts as constituting multiple {{ companyData?.name.toUpperCase() }} Accounts, and suspend or close all such {{ companyData?.name.toUpperCase() }} Accounts. {{ companyData?.name.toUpperCase() }} will also be entitled to retain the funds till the Player proves that he did not attempt to create multiple accounts.</p>
          <p>4.11 If you notice that you have more than one registered {{ companyData?.name.toUpperCase() }} Account you must notify us immediately. Failure to do so may lead to your {{ companyData?.name.toUpperCase() }} Account being blocked and the funds retained.</p>
          ...
          <p>4.13 We are not liable or responsible for any abuse or misuse of your {{ companyData?.name.toUpperCase() }} Account by third parties due to your disclosure, whether intentional, accidental, active or passive, of your login details to any third party.</p>
          <p>4.14 You are prohibited from selling, transferring or acquiring {{ companyData?.name.toUpperCase() }} Accounts to or from other Players.</p>
          ...
          <p>c) {{ companyData?.name.toUpperCase() }} is requested to do so by the police, any regulatory authority or court or if {{ companyData?.name.toUpperCase() }} is unable to verify Your identity, profession or source of funds as is expressly required by the applicable regulations;</p>
          <p>d) You are in breach of these Terms and Conditions, the applicable regulations or the fair use of our services, or {{ companyData?.name.toUpperCase() }} has concerns that You are a compulsive problem gambler without being self-excluded;</p>
          <p>e) {{ companyData?.name.toUpperCase() }} considers that any of the events referred to in (a) to (e) above may have occurred or are likely to occur.</p>
          <p>4.20 If {{ companyData?.name.toUpperCase() }} closes or suspends Your {{ companyData?.name.toUpperCase() }} Account for any of the reasons referred to in 4.19, You shall, to the extent permitted by applicable laws, be liable for any and all claims, direct losses, liabilities, damages, costs and expenses incurred or suffered by {{ companyData?.name.toUpperCase() }} (together, the “Claims”) arising therefrom and shall, to the extent permitted by applicable laws, indemnify and hold {{ companyData?.name.toUpperCase() }} harmless on demand for such Claims.</p>
          <p>4.21 In the circumstances referred to in 4.19, {{ companyData?.name.toUpperCase() }} shall also be entitled to void any bets placed by You following such actions by You or to withhold and/or retain any and all amounts which would otherwise have been paid or payable to you (including any winnings) to the extent permitted by law.</p>
          ...
          <p>4.23 If {{ companyData?.name.toUpperCase() }} closes Your {{ companyData?.name.toUpperCase() }} Account it will inform You of the available means to withdraw the remaining balance on Your {{ companyData?.name.toUpperCase() }} Account.</p>
          <p>Our Liability</p>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>j) There is a risk of losing cryptocurrency when using the Service and that {{ companyData?.name.toUpperCase() }} has no responsibility to you for any such loss;</p>
          ...
          <p>q) {{ companyData?.name.toUpperCase() }} shall not be treated as a financial institution;</p>
          <p>Others</p>
          <p>r) Your use of the Service is at your sole option, discretion and risk;</p>
          <p>s) You will not conduct criminal activities through the {{ companyData?.name.toUpperCase() }} Account;</p>
          ...
          <p>x) The telecommunications networks and Internet access services required for you to access and use the Service are entirely beyond the control of {{ companyData?.name.toUpperCase() }} and {{ companyData?.name.toUpperCase() }} shall have no liability whatsoever for any outages, slowness, capacity constraints or other deficiencies affecting the same;</p>
          ...
          <p>5.2 In case of a breach of any of the representations, warrants or covenants mentioned in 5.1, {{ companyData?.name.toUpperCase() }} reserves the right to close or suspend Your {{ companyData?.name.toUpperCase() }} account at its own discretion and void any bets to the extent applicable by law.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>6.1 {{ companyData?.name.toUpperCase() }} warrants that they will:</p>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>a) Funded/are funding Your {{ companyData?.name.toUpperCase() }} Account using third party payment methods; and/or</p>
          ...
          <p>7.11 We reserve the right to use additional procedures and means to verify your identity when processing deposits into your {{ companyData?.name.toUpperCase() }} Account.</p>
        ...
        <div class="terms-conditions-describe">
          <p>8.1 {{ companyData?.name.toUpperCase() }} reserves the right to refuse any withdrawal by a Player from their {{ companyData?.name.toUpperCase() }} Account until:</p>
          <p>a) the Player’s identity has been verified and {{ companyData?.name.toUpperCase() }} has confirmed the withdrawal is being made by a holder of the {{ companyData?.name.toUpperCase() }} Account;</p>
          <p>b) the withdrawal is being transferred to an account of which the Player is a legal holder;</p>
          <p>c) any additional information requested by {{ companyData?.name.toUpperCase() }} has been provided; and</p>
          ...
          <p>8.8 If You have multiple withdrawals pending, {{ companyData?.name.toUpperCase() }} reserves the right to reject all withdrawals and request You to perform one withdrawal with the sum of all multiple withdrawals.</p>
          <p>8.9 {{ companyData?.name.toUpperCase() }} will not charge You any fees on withdrawals. However, You are advised to check if the payment method You use imposes any additional charges. {{ companyData?.name.toUpperCase() }} will not be responsible for any commissions or processing fees charged to You by third parties such as Your bank as this is beyond our control.</p>
        ...
        <div class="terms-conditions-describe">
          <p>15.1 Your computer equipment or mobile device and internet connection may affect the performance and/or operation of the Website. {{ companyData?.name.toUpperCase() }} does not guarantee that the Website will operate without faults or errors or that {{ companyData?.name.toUpperCase() }} services will be provided without interruption. {{ companyData?.name.toUpperCase() }} does not accept any liability for any failures or issues that arise due to Your equipment, internet connection or internet or telecommunication service provider (including, for example, if You are unable to place bets or wagers or to view or receive certain information in relation to particular events).</p>
          <p>15.2 For customers using a mobile device for the placing of bets/wagers, please note that {{ companyData?.name.toUpperCase() }} will not be responsible for any damage to, or loss of data from the mobile device that the software is installed on, and will also not be responsible for any call, data or other charges incurred whilst using the software.</p>
        ...
        <div class="terms-conditions-describe">
          <p>17.1 In order for You to use the Website and Services, You may need to download some software (for example, casino games that are made available via a flash player). Also, certain third party product providers may require You to agree to additional terms and conditions governing the use of their products that are available on or through the Website. If You do not accept those third party terms and conditions, do not use the relevant third party software. {{ companyData?.name.toUpperCase() }} does not accept any liability in respect of any third party software.</p>
        ...
        <div class="terms-conditions-describe">
          <p>19.1 {{ companyData?.name.toUpperCase() }} will not be liable in the event You try to or obtain an advantage from any errors in respect of bets or wagers on the Website if You were deliberately acting in bad-faith, including where: (i) there is an obvious error in the relevant odds, spreads, handicap, totals, cash-out; (ii) {{ companyData?.name.toUpperCase() }} continues to accept bets or wagers on closed or suspended markets; (iii) {{ companyData?.name.toUpperCase() }} incorrectly calculates or pays a settlement amount, including where a bet is Cashed Out for the full settlement amount, or a bet is made void incorrectly, where ‘Void if player does not start’ was selected at bet placement; or (iv) any error occurs in a random number generator or pay tables included, incorporated or used in any game or product.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>20.1 Without prejudice to any other rights, if a User breaches in whole or in part any provision contained herein, {{ companyData?.name.toUpperCase() }} reserves the right to take such action as it sees fit, including terminating this Agreement or any other agreement in place with the User and/or taking legal action against such User.</p>
          <p>20.2 You agree to fully indemnify, defend and hold harmless {{ companyData?.name.toUpperCase() }} and its shareholders, directors, agents and employees from and against all claims, demands, liabilities, damages, losses, costs and expenses, including legal fees and any other charges whatsoever, howsoever caused, that may arise as a result of:</p>
        ...
        <div class="terms-conditions-describe">
          <p>27.1 {{ companyData?.name.toUpperCase() }} reserves the right to assign this agreement, in whole or in part, at any time without notice. The User may not assign any of his/her rights or obligations under this Agreement.</p>
        </div>
