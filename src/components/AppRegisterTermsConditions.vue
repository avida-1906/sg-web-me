      ...
      <div>
        ...
        <div class="terms-conditions-describe">
          <p>
            ...
            enter into an agreement with {{ companyData?.name }}.
          </p>
          <p>
            3.2 This Agreement should be read by You in its entirety prior to your
            use of {{ companyData?.name }}'s service or products.
            Please note that the Agreement constitutes a legally binding
            agreement between you and {{ companyData?.name }}.
          </p>
          <p>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>4.5 {{ companyData?.name }} will not accept registration from individuals:</p>
          ...
          <p>c) {{ companyData?.name }} is requested to do so by the police, any regulatory authority or court or if {{ companyData?.name }} is unable to verify Your identity, profession or source of funds as is expressly required by the applicable regulations;</p>
          <p>d) You are in breach of these Terms and Conditions, the applicable regulations or the fair use of our services, or {{ companyData?.name }} has concerns that You are a compulsive problem gambler without being self-excluded;</p>
          <p>e) {{ companyData?.name }} considers that any of the events referred to in (a) to (e) above may have occurred or are likely to occur.</p>
          <p>4.20 If {{ companyData?.name }} closes or suspends Your {{ companyData?.name }} Account for any of the reasons referred to in 4.19, You shall, to the extent permitted by applicable laws, be liable for any and all claims, direct losses, liabilities, damages, costs and expenses incurred or suffered by {{ companyData?.name }} (together, the “Claims”) arising therefrom and shall, to the extent permitted by applicable laws, indemnify and hold {{ companyData?.name }} harmless on demand for such Claims.</p>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>x) The telecommunications networks and Internet access services required for you to access and use the Service are entirely beyond the control of {{ companyData?.name }} and {{ companyData?.name }} shall have no liability whatsoever for any outages, slowness, capacity constraints or other deficiencies affecting the same;</p>
          ...
          <p>5.2 In case of a breach of any of the representations, warrants or covenants mentioned in 5.1, {{ companyData?.name }} reserves the right to close or suspend Your {{ companyData?.name }} account at its own discretion and void any bets to the extent applicable by law.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>6.2 The software is provided "as is" without any warranties, conditions, undertakings or representations, express or implied, statutory or otherwise. {{ companyData?.name }} hereby excludes all implied terms, representations, conditions and warranties (including any of merchantability, merchantable quality, satisfactory quality and fitness for any particular purpose). {{ companyData?.name }} does not warrant that: (i) the Website and Services will meet Your requirements; (ii) the Website and Services will not infringe any third party’s intellectual property rights; (iii) the operation of the Website and Services will be error-free or uninterrupted; (iv) any defects in the Website and Services will be corrected; or (v) the Website or the servers are virus-free.</p>
          <p>
            6.3 {{ companyData?.name }} can be contacted by email on
            <a class="link-inline"><span>support@{{ companyData?.name.toLowerCase() }}.com</span><BaseIcon name="uni-jump-page" /></a>
            or on its live chat 24/7. The live chat is available when logged into Your {{ companyData?.name }} Account. In the event that {{ companyData?.name }}, in its sole discretion, deems that Your behaviour, via live chat, email, or otherwise, has been abusive or derogatory towards any of {{ companyData?.name }}’s or its Affiliates or third-party service provider’s employees, {{ companyData?.name }} shall have the right to block or terminate Your {{ companyData?.name }} Account.
          </p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>7.1 You may participate in any Game only if you have sufficient funds on your {{ companyData?.name }} Account for such participation. For that purpose you shall use the payment methods available on the Website to deposit your funds. {{ companyData?.name }} will not give you any credit whatsoever for participation in any Game.</p>
          <p>
            7.2 You must deposit funds to Your {{ companyData?.name }} Account using the payment methods available at
            <a class="link-inline"><span>{{ location.hostname }}</span><BaseIcon name="uni-jump-page" /></a>
            .
          </p>
          <p>7.3 You shall ensure that funds that You deposit into your {{ companyData?.name }} Account are not tainted with any illegality and, in particular, do not originate from any illegal activity or source.</p>
          <p>7.4 To deposit funds into your {{ companyData?.name }} Account, you can transfer funds from crypto-wallets under your control or through any other payment methods available on <a class="link-inline"><span>{{ location.hostname }}</span><BaseIcon name="uni-jump-page" /></a>. Deposits can only be made with your own funds.</p>
          <p>7.5 You should only deposit money into Your account for the purpose of You using such money to place bets/wager on the Website. {{ companyData?.name }} is entitled to suspend or close Your account if we reasonably believe that You are depositing funds without any intention to place sporting and/or gaming wagers. In such circumstances we may also report this activity to relevant authorities.</p>
          <p>7.6 You acknowledge and understand that funding Your {{ companyData?.name }} Account can only be funded by payment methods owned by You.</p>
          <p>7.7 You further understand, agree and acknowledge that if {{ companyData?.name }} discovers, detects and/or identifies that You:</p>
          <p>a) Funded/are funding Your {{ companyData?.name }} Account using third party payment methods; and/or</p>
          <p>b) Funded/are funding Your {{ companyData?.name }} Account with funds that are tainted with illegality, such activity will be deemed as constituting a violation of the Terms of Service amounting to fraud, and by extension:</p>
          <p>i) {{ companyData?.name }} reserves the right, at its own discretion, to suspend or close Your {{ companyData?.name }} Account; and</p>
          <p>ii) {{ companyData?.name }} reserves the right, at its own discretion, to cancel, reverse or adjust any transactions and to forfeit funds deposited and/or winnings generated from the deposited funds.</p>
          <p>7.8 {{ companyData?.name }} can set at its own discretion a minimum deposit amount. The minimum deposit amount can be changed at all time at {{ companyData?.name }}’s discretion and will be identified on the website. Please be aware that depending on the payment method used by You, additional fees might be charged by the payment providers.</p>
          ...
          <p>7.10 Deposits are immediately processed and the updated balance is shown in the {{ companyData?.name }} Account instantly whenever a payment service provider is used. {{ companyData?.name }} does not take responsibility for any delays caused due to its payment system or due to delays caused by any third party. Note that:</p>
          ...
          <p>7.11 We reserve the right to use additional procedures and means to verify your identity when processing deposits into your {{ companyData?.name }} Account.</p>
          <p>7.12 Funds cannot be transferred from your {{ companyData?.name }} Account to the {{ companyData?.name }} Account of another Player.</p>
          <p>
            7.13 {{ companyData?.name }} can refuse any deposits at its own discretion. Users that have their accounts blocked, or suspended shall refrain from depositing at
            <a class="link-inline"><span>{{ location.hostname }}</span><BaseIcon name="uni-jump-page" /></a>
            .
          </p>
          <p>7.14 In the event that a player tries to deposit when his account is blocked or suspended, {{ companyData?.name }} will have the right to retain the funds.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>8.1 {{ companyData?.name }} reserves the right to refuse any withdrawal by a Player from their {{ companyData?.name }} Account until:</p>
          <p>a) the Player’s identity has been verified and {{ companyData?.name }} has confirmed the withdrawal is being made by a holder of the {{ companyData?.name }} Account;</p>
          <p>b) the withdrawal is being transferred to an account of which the Player is a legal holder;</p>
          <p>c) any additional information requested by {{ companyData?.name }} has been provided; and</p>
        ...
        <div class="terms-conditions-describe">
          <p>10.1 You are allowed to place Your bets/wagers on the markets/products offered in the Website. {{ companyData?.name }} is not obliged to accept any bet/wager from You and bets/wagers will only be deemed as valid and finalised, and therefore as accepted by {{ companyData?.name }}, when You receive the confirmation from {{ companyData?.name }} of the acceptance of Your bet/wager.</p>
          <p>10.2 {{ companyData?.name }} only accepts bets/wagers made online (including via mobile device). Bets/wagers are not accepted in any other form (post, email, fax, etc.) and where received will be invalid and void - win or lose.</p>
          <p>10.3 It is Your responsibility to ensure details of the bets/wagers are correct. Once bets/wagers have been placed they may not be cancelled by You. Bets can only be changed by You using our Edit Bet feature, where this is available. {{ companyData?.name }} can only cancel or amend a bet/wager if the relevant event has been suspended or cancelled, if there was an obvious error on the relevant bet or its odds, if the bet is placed in breach of the Terms or if required to do so for legal or regulatory reasons.</p>
          <p>10.4 Your funds will be allocated to bets/wagers in the order they are placed and will not be available for any other use. {{ companyData?.name }} reserves the right to void and/or reverse any transactions made after a bet/wager has been placed involving allocated funds, either at the time or retrospectively.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>
            11.2 A bet/wager that You request will only be valid once accepted by {{ companyData?.name }}. Each valid bet/wager will receive a unique transaction code. We shall not be liable for the settlement of any bets/wagers which are not issued with a unique transaction code. If You are unsure about the validity of a bet/wager, please check Your account history, or contact our Customer Support Team
            (<a class="link-inline"><span>support@{{ companyData?.name.toLowerCase() }}.com</span><BaseIcon name="uni-jump-page" /></a>).
          </p>
          <p>11.3 Should a dispute arise regarding the content of a bet, You and {{ companyData?.name }} agree that the {{ companyData?.name }} transaction log database of its internal control system will be the ultimate authority in such matters.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>15.1 Your computer equipment or mobile device and internet connection may affect the performance and/or operation of the Website. {{ companyData?.name }} does not guarantee that the Website will operate without faults or errors or that {{ companyData?.name }} services will be provided without interruption. {{ companyData?.name }} does not accept any liability for any failures or issues that arise due to Your equipment, internet connection or internet or telecommunication service provider (including, for example, if You are unable to place bets or wagers or to view or receive certain information in relation to particular events).</p>
          <p>15.2 For customers using a mobile device for the placing of bets/wagers, please note that {{ companyData?.name }} will not be responsible for any damage to, or loss of data from the mobile device that the software is installed on, and will also not be responsible for any call, data or other charges incurred whilst using the software.</p>
          <p>15.3 Due to limited display sizes on mobile devices, the mobile experience might differ slightly from other platforms. Differences might include, but are not limited to, the location of certain information on the platform and game names not being visible on all game pages.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          ...
          <p>16.2 You must not use the Website for the benefit of a third party or for any purpose which is illegal, defamatory, abusive or obscene, or which {{ companyData?.name }} considers discriminatory, fraudulent, dishonest or inappropriate. {{ companyData?.name }} may report to the authorities any activity which it considers to be suspicious and/or in breach of this paragraph.</p>
          <p>16.3 If {{ companyData?.name }} has a reasonable suspicion that You are involved in fraudulent, dishonest or criminal acts, as set out under applicable laws, via or in connection with the Website or Services, {{ companyData?.name }} may seek criminal and contractual sanctions against You. {{ companyData?.name }} will withhold payment to any customer where any of these are suspected or where the payment is suspected to be for the benefit of a third party.</p>
          <p>16.4 You shall indemnify and shall be liable to pay {{ companyData?.name }}, on demand, all costs, charges or losses sustained or incurred by us and our affiliates (including any direct, indirect or consequential losses, loss of profit and loss of reputation) in respect of all Claims arising directly or indirectly from Your fraudulent, dishonest or criminal acts while using the Website or Services</p>
        ...
        <div class="terms-conditions-title">
          17. SOFTWARE AND TECHNOLOGY ISSUES
        </div>
        <div class="terms-conditions-describe">
          <p>17.1 In order for You to use the Website and Services, You may need to download some software (for example, casino games that are made available via a flash player). Also, certain third party product providers may require You to agree to additional terms and conditions governing the use of their products that are available on or through the Website. If You do not accept those third party terms and conditions, do not use the relevant third party software. {{ companyData?.name }} does not accept any liability in respect of any third party software.</p>
          <p>17.2 You are only permitted to use any and all software made available to You via the Website for the purpose of using the Website and Services and, save to the extent permitted by applicable law, for no other purposes whatsoever.</p>
          <p>17.3 We hereby grant to You a personal, non-exclusive, non-transferable right to use the Website for the sole purpose of accessing and using the Services on the Website, in accordance with these Terms and Conditions. This right to use our Website and will be immediately terminated once Your user registration is cancelled for any reason, and specially, but not limited to, if You make use of that right with the aim of generating a parallel enterprise based in our Website or our products, or with the aim of making use of an automated service or software analysing, capturing or somehow using the information shown in our Website.</p>
          ...
          <p>c) enter, access or attempt to enter or access or otherwise bypass {{ companyData?.name }}’s security system or interfere in any way (including but not limited to, robots or similar devices) with the products or the Website or attempt to make any modifications to the software and/or any features or components thereof;</p>
          ...
          <p>17.5 You do not own the software that forms part of the Website. Such software is owned and is the exclusive property of {{ companyData?.name }} or a third party software provider company (any such third party provider, the "Software Provider"). Any software and accompanying documentation which have been licensed to {{ companyData?.name }} are proprietary products of the Software Provider and protected throughout the world by copyright law. Your use of the software does not give You ownership of any intellectual property rights in the software.</p>
          <p>17.6 The software is provided "as is" without any warranties, conditions, undertakings or representations, express or implied, statutory or otherwise. {{ companyData?.name }} hereby excludes all implied terms, conditions and warranties, including any of merchantability, merchantable quality, satisfactory quality and fitness for any particular purpose, completeness or accuracy of the services or the software or infringement of applicable laws and regulations. {{ companyData?.name }} does not warrant or condition that: (i) the software will meet Your requirements; (ii) the software will not infringe any third party’s intellectual property rights; (iii) the operation of the software will be error free or uninterrupted; (iv) any defects in the software will be corrected; or (v) the software or the servers are virus-free.</p>
          <p>17.7 In the event of communications or system errors occurring in connection with the settlement of accounts or other features or components of the software, neither {{ companyData?.name }} nor the Software Provider will have any liability to You or to any third party in respect of such errors. {{ companyData?.name }} reserves the right in the event of such errors to remove all relevant products from the Website and take any other action to correct such errors.</p>
          <p>17.8 You hereby acknowledge that how You use the software is outside of {{ companyData?.name }}’s control. Accordingly, You install and/or use the software at Your own risk. {{ companyData?.name }} will not have any liability to You or to any third party in respect of Your receipt of and/or use of the software.</p>
          <p>17.9 The software may include confidential information which is secret and valuable to the Software Provider and/or {{ companyData?.name }}. You are not entitled to use or disclose that confidential information other than strictly in accordance with these Terms and Conditions.</p>
          <p>17.10 {{ companyData?.name }} shall not be liable if for any reason the Website is unavailable at any time or for any period. We reserve the right to make changes or corrections to or to alter, suspend or discontinue any aspect of the Website and the content or services or products available through it, including Your access to it.</p>
        ...
        <div class="terms-conditions-describe">
          <p>19.1 {{ companyData?.name }} will not be liable in the event You try to or obtain an advantage from any errors in respect of bets or wagers on the Website if You were deliberately acting in bad-faith, including where: (i) there is an obvious error in the relevant odds, spreads, handicap, totals, cash-out; (ii) {{ companyData?.name }} continues to accept bets or wagers on closed or suspended markets; (iii) {{ companyData?.name }} incorrectly calculates or pays a settlement amount, including where a bet is Cashed Out for the full settlement amount, or a bet is made void incorrectly, where ‘Void if player does not start’ was selected at bet placement; or (iv) any error occurs in a random number generator or pay tables included, incorporated or used in any game or product.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>20.1 Without prejudice to any other rights, if a User breaches in whole or in part any provision contained herein, {{ companyData?.name }} reserves the right to take such action as it sees fit, including terminating this Agreement or any other agreement in place with the User and/or taking legal action against such User.</p>
          <p>20.2 You agree to fully indemnify, defend and hold harmless {{ companyData?.name }} and its shareholders, directors, agents and employees from and against all claims, demands, liabilities, damages, losses, costs and expenses, including legal fees and any other charges whatsoever, howsoever caused, that may arise as a result of:</p>
        ...
        <div class="terms-conditions-describe">
          <p>21.1 Under no circumstances, including negligence, shall {{ companyData?.name }} be liable for any special, incidental, direct, indirect or consequential damages whatsoever (including, without limitation, damages for loss of business profits, business interruption, loss of business information, or any other pecuniary loss) arising out of the use (or misuse) of the Service even if {{ companyData?.name }} had prior knowledge of the possibility of such damages.</p>
          <p>21.2 Nothing in this Agreement shall exclude or limit {{ companyData?.name }}'s liability for death or personal injury resulting from its negligence.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>22.1 {{ companyData?.name }} and its licensors are the sole holders of all rights in and to the Service and code, structure and organisation, including copyright, trade secrets, intellectual property and other rights. You may not, within the limits prescribed by applicable laws: (a) copy, distribute, publish, reverse engineer, decompile, disassemble, modify, or translate the website; or (b) use the Service in a manner prohibited by applicable laws or regulations (each of the above is an "Unauthorised Use"). {{ companyData?.name }} reserves any and all rights implied or otherwise, which are not expressly granted to the User hereunder and retain all rights, title and interest in and to the Service. You agree that you will be solely liable for any damage, costs or expenses arising out of or in connection with the commission by you of any Unauthorized Use. You shall notify {{ companyData?.name }} immediately upon becoming aware of the commission by any person of any Unauthorised Use and shall provide {{ companyData?.name }} with reasonable assistance with any investigations it conducts in light of the information provided by you in this respect.</p>
          <p>22.2 The term "{{ companyData?.name }}", its domain names and any other trade marks, or service marks used by {{ companyData?.name }} as part of the Service (the "Trade Marks"), are solely owned by {{ companyData?.name }}. In addition, all content on the website, including, but not limited to, the images, pictures, graphics, photographs, animations, videos, music, audio and text (the "Site Content") belongs to {{ companyData?.name }} and is protected by copyright and/or other intellectual property or other rights. You hereby acknowledge that by using the Service, you obtain no rights in the Site Content and/or the Trade Marks, or any part thereof. Under no circumstances may you use the Site Content and/or the Trade Marks without {{ companyData?.name }}'s prior written consent. Additionally, you agree not to do anything that will harm or potentially harm the rights, including the intellectual property rights of {{ companyData?.name }}.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>
            23.1 If a User wishes to make a complaint, please contact {{ companyData?.name }}'s customer service team at
            <a class="link-inline"><span>support@{{ companyData?.name.toLowerCase() }}.com</span><BaseIcon name="uni-jump-page" /></a>
        ...
        <div class="terms-conditions-describe">
          <p>27.1 {{ companyData?.name }} reserves the right to assign this agreement, in whole or in part, at any time without notice. The User may not assign any of his/her rights or obligations under this Agreement.</p>
        </div>
        ...
        <div class="terms-conditions-describe">
          <p>28.1 No waiver by {{ companyData?.name }} of any breach of any provision of this Agreement (including the failure of {{ companyData?.name }} to require strict and literal performance of or compliance with any provision of this Agreement) shall in any way be construed as a waiver of any subsequent breach of such provision or of any breach of any other provision of this Agreement.</p>
          ...
          <p>28.3 Nothing in this Agreement shall create or be deemed to create a partnership, agency, trust arrangement, fiduciary relationship or joint venture between you and {{ companyData?.name }}.</p>
          <p>28.4 {{ companyData?.name }} may assign, transfer, charge, sub-license, or deal in any other manner with this Agreement, or sub-contract any of its rights and obligations under this Agreement, to any other party.</p>
          <p>28.5 This Agreement constitutes the entire understanding and agreement between you and {{ companyData?.name }} regarding the Service and supersedes any prior agreement, understanding, or arrangement between you and {{ companyData?.name }}.</p>
        </div>
